...
from app.services.gis_preflight import repair_gis_coverage
...
async def single_link_plan(
    ...
    gis_result = await asyncio.to_thread(repair_gis_coverage, request_body)
    if gis_result.blocks_planning:
        raise HTTPException(status_code=400, detail=gis_result.error_message)
...
async def batch_plan(request_body: BatchPlanRequest, http_request: Request, db: Session = Depends(get_db)) -> BatchPlanResult:
    ...
    for site_index, site in enumerate(request_body.sites):
        if await http_request.is_disconnected():
            break
        gis_result = await asyncio.to_thread(repair_gis_coverage, site)
